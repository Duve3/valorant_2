...
class connection:
    def __init__(self,addr) -> None:
        ...
        while self.id == "":
                    self.id = self.messagehandle()

        print(f"ID:{self.id}")
        self.send(self.id)
        thread = threading.Thread(target=self.messagehandlefull)
        thread.start()
    ...
    def sendasjson(self, msg):
        msgjson = json.dumps(msg, ensure_ascii=False)  # Serialize the dictionary as JSON
    ...
    def messagehandle(self):
                ...
                msg = None
                LONGNESS = self.client.recv(HEADER).decode(FORMAT)#must be less then 64 thingy
                try:
                    LONGNESS = int(LONGNESS)
                except:
                    print(LONGNESS)
                if not LONGNESS == "":
                    msg = self.client.recv(LONGNESS).decode(FORMAT)
                return msg
    def messagehandlefull(self):
        while True:
                msg = self.messagehandle()
connect = connection(addr)    
import pygame
...
class display:
    ...
    def bulletsdraw(self, layer):
        ...
        if Player.gun.reloading:
                 ammo = font.render("Reloading...", True, (255,0,0))
                 self.screen.blit(ammo, (size-300,size-50))
        else:
...
def Main():
    ...
    while running:
        ...
        data = [Player.x,Player.y,connect.id]



        connect.sendasjson(data)
